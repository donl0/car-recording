...
async def to_handler(bot: Bot, dp: Dispatcher):
    ...
    async def get_time_for_sheep(message: types.Message, state: FSMContext):
        ...
        await bot.send_message(chat_id=id_person, text='Введите название машины')
        await OrderDataUser.brand_wait.set()

    @dp.message_handler(state=OrderDataUser.brand_wait)
    ...
    @dp.message_handler(state=OrderDataUser.distant_of_car)
    ...
    @dp.message_handler(state=OrderDataUser.action_wait)
